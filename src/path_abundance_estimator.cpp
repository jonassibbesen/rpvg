...
PathAbundances PathAbundanceEstimator::inferPathClusterAbundances(const vector<ReadPathProbabilities> & cluster_probs, const vector<PathInfo> & cluster_paths) {
    ...
    if (!cluster_probs.empty()) {
        ...
        PathLikelihoodEstimator::constructProbabilityMatrix(&read_path_probs, &noise_probs, &read_counts, cluster_probs);
        PathLikelihoodEstimator::addNoiseToProbabilityMatrix(&read_path_probs, noise_probs);

        PathLikelihoodEstimator::sortProbabilityMatrix(&read_path_probs, &read_counts);
        PathLikelihoodEstimator::collapseProbabilityMatrix(&read_path_probs, &read_counts, prob_precision);

        PathAbundances path_abundances(cluster_paths, true, false);

        expectationMaximizationEstimator(&(path_abundances), read_path_probs, read_counts);
        removeNoiseAndRenormalizeAbundances(&(path_abundances));
...
PathAbundances MinimumPathAbundanceEstimator::inferPathClusterAbundances(const vector<ReadPathProbabilities> & cluster_probs, const vector<PathInfo> & cluster_paths) {
    ...
    if (!cluster_probs.empty()) {
        ...
        PathLikelihoodEstimator::addNoiseToProbabilityMatrix(&min_path_read_path_probs, noise_probs);

        PathLikelihoodEstimator::sortProbabilityMatrix(&min_path_read_path_probs, &read_counts);
        PathLikelihoodEstimator::collapseProbabilityMatrix(&min_path_read_path_probs, &read_counts, prob_precision);
        ...
        path_abundances.read_count = read_counts.sum();

        for (size_t j = 0; j < min_path_cover.size(); j++) {

            path_abundances.confidence(min_path_cover.at(j)) = min_path_abundances.confidence(j);
            path_abundances.expression(min_path_cover.at(j)) = min_path_abundances.expression(j);
        }

        assert(min_path_abundances.confidence.cols() == min_path_cover.size() + 1);

        path_abundances.confidence(min_path_cover.size()) = min_path_abundances.confidence(min_path_cover.size());
        path_abundances.expression(min_path_cover.size()) = min_path_abundances.expression(min_path_cover.size());  
...
PathAbundances NestedPathAbundanceEstimator::inferPathClusterAbundances(const vector<ReadPathProbabilities> & cluster_probs, const vector<PathInfo> & cluster_paths) {
    ...
    if (!cluster_probs.empty()) {
        ...
        for (auto & path_indices_sample: ploidy_path_indices_samples) {
            ...
            PathLikelihoodEstimator::addNoiseToProbabilityMatrix(&ploidy_read_path_probs, noise_probs);

            auto ploidy_read_counts = read_counts;
            PathLikelihoodEstimator::sortProbabilityMatrix(&ploidy_read_path_probs, &ploidy_read_counts);
            PathLikelihoodEstimator::collapseProbabilityMatrix(&ploidy_read_path_probs, &ploidy_read_counts, prob_precision);
            ...
            for (size_t i = 0; i < path_indices_sample.first.size(); i += 2) {
                ...
                if (ploidy_abundances.confidence(i) > 0) {
                    ...
                    path_abundances.confidence(path_indices_sample.first.at(i)) += (ploidy_abundances.confidence(i) * path_indices_sample.second);
                    path_abundances.expression(path_indices_sample.first.at(i)) += (ploidy_abundances.expression(i) * path_indices_sample.second);
                }
            }
            ...
            if (ploidy_abundances.confidence(path_indices_sample.first.size()) > 0) {
                ...
                path_abundances.confidence(cluster_paths.size()) += (ploidy_abundances.confidence(path_indices_sample.first.size()) * path_indices_sample.second);
                path_abundances.expression(cluster_paths.size()) += (ploidy_abundances.expression(path_indices_sample.first.size()) * path_indices_sample.second);  
            }
        }

        for (size_t i = 0; i < path_abundances.expression.cols(); ++i) {

            if (path_abundances.confidence(i) > 0) {

                path_abundances.expression(i) /= path_abundances.confidence(i);
            }

            path_abundances.confidence(i) /= num_nested_its;
        }
