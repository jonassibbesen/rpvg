...
void PathAbundanceEstimator::gibbsAbundanceSampler(PathClusterEstimates * path_cluster_estimates, const Eigen::ColMatrixXd & read_path_probs, const Eigen::RowVectorXui & read_counts, const double gamma, mt19937 * mt_rng) const {

    assert(!path_cluster_estimates->gibbs_abundance_samples.empty());
    assert(path_cluster_estimates->gibbs_abundance_samples.back().path_ids.size() == path_cluster_estimates->abundances.cols());
    assert(path_cluster_estimates->gibbs_abundance_samples.back().samples.size() == path_cluster_estimates->abundances.cols());
    ...
    for (uint32_t gibbs_it = 1; gibbs_it <= num_gibbs_its; ++gibbs_it) {
        ...
        if (gibbs_it % gibbs_thin_its == 0) {
            ...
            for (size_t i = 0; i < gibbs_abundances.cols(); ++i) {
                ...
                path_cluster_estimates->gibbs_abundance_samples.back().samples.at(i).emplace_back(gibbs_abundances(0, i));
            }
        }
    }
}
...
void PathAbundanceEstimator::removeNoiseAndRenormalizeAbundances(PathClusterEstimates * path_cluster_estimates) const {
    ...
    for (auto & abundance_samples: path_cluster_estimates->gibbs_abundance_samples) {
...
void PathAbundanceEstimator::updateEstimates(PathClusterEstimates * path_cluster_estimates, const PathClusterEstimates & new_path_cluster_estimates, const vector<uint32_t> & path_indices, const uint32_t sample_count) const {
   ...
   for (size_t i = 0; i < path_indices.size(); ++i) {
    ...
    if (!new_path_cluster_estimates.gibbs_abundance_samples.empty()) {
       ...
       assert(new_path_cluster_estimates.gibbs_abundance_samples.size() == 1);
       path_cluster_estimates->gibbs_abundance_samples.emplace_back(move(new_path_cluster_estimates.gibbs_abundance_samples.front()));
    } 
}
...
void NestedPathAbundanceEstimator::inferAbundance(PathClusterEstimates * path_cluster_estimates, const vector<ReadPathProbabilities> & cluster_probs, mt19937 * mt_rng) {
    ...
    if (!cluster_probs.empty()) {
        ...
        spp::sparse_hash_map<<vector<uint32_t>, uint32_t> collapsed_ploidy_path_indices_samples;
...
vector<vector<uint32_t> > NestedPathAbundanceEstimator::findPathSourceGroups(const vector<PathInfo> & paths) const {

    vector<vector<uint32_t> > path_source_groups;    
        
    auto path_groups = findPathGroups(path_cluster_estimates->paths);
    assert(!path_groups.empty());

    vector<vector<uint32_t> > group_source_ids;

    for (auto & path_idx: path_groups.front()) {

        path_source_groups.emplace_back(vector<uint32_t>(1, path_idx));
        group_source_ids.emplace_back(paths.at(path_idx).source_ids);
    }

    for (size_t i = 1; i < path_groups.size(); ++i) {
...
void NestedPathAbundanceEstimator::samplePloidyPathIndices(vector<vector<uint32_t> > * ploidy_path_indices_samples, const PathClusterEstimates & group_path_cluster_estimates, const vector<uint32_t> & group, mt19937 * mt_rng) {
    ...
    for (size_t i = 0; i < num_nested_samples; ++i) {
        ...
        vector<uint32_t> sampled_path_indices = group_path_cluster_estimates.path_group_sets.at(group_ploidy_path_sampler(*mt_rng));
